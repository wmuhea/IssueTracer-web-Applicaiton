...
function appendIssue(fetchedIssue) {
    ...
    // Title
    let issueDetails =  $('<h6 class="border-bottom border-gray pb-2 mb-0">').html($('<strong>').html("@IssueId"));
    ...
    let separator = $('<small class= "d-block mt-3">')
...
function updateWelcomeNote (fetchedIssue) {
    let profileButton = $('<button type="button" class="btn btn-primary " width="50px" height= "50px" >');
    let profileLink = $('<a href="/profile" target ="_blank">').html(fetchedIssue.assignedTo);
    profileButton.html(profileLink);
    $('#profileNote').append(profileButton);
}
